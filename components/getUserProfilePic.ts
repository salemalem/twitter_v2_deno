...
export default async function getUserDescription(
  twitterID: string,
): Promise<string> {
  const twitterUserInfo = await fetch(`https://api.twitter.com/2/users/${twitterID}?user.fields=profile_image_url`, {
    headers: {
  ...
  return twitterUserInfoJson.data.description;
}
